...
const DashboardMap = ({
  ...
  const [isDeliveryCompletionInProgress, setIsDeliveryCompletionInProgress] =
    useState(false); // Track completion flow
  ...
  const [showCompleteDeliveryPopup, setShowCompleteDeliveryPopup] =
    useState(false);

  // Popup protection state - prevents popups from closing due to external state changes
  const [isPopupInteractionActive, setIsPopupInteractionActive] =
    useState(false);
  ...
  const currentDeliveryDestination = useMemo(() => {
    ...
    currentDelivery,
    deliveryStatus,
    deliveryPickupCoords,
    deliveryDropoffCoords,
  ]);
  ...
  useEffect(() => {
    if (activeTrip?.status) {
      ...
      if (activeTrip.status !== "dropoff_arrived") {
        ...
        console.log(
          "Skipping local status update for dropoff_arrived (buyer notification only)"
        );
      }
    }
  }, [activeTrip?.status]);
  ...
  useEffect(() => {
    ...
    if (isPopupInteractionActive) {
      console.log(
        "🔒 Skipping delivery state update - popup interaction active"
      );
      return;
    }
    ...
    if (activeTrip && activeTrip.isDelivery) {
      ...
      setCurrentDelivery({
        ...
        pickup: {
          ...
          coordinates: [
            activeTrip.pickup.location.coordinates[0],
            activeTrip.pickup.location.coordinates[1],
          ],
        },
        dropoff: {
          address: activeTrip.dropoff.address,
          coordinates: [
            activeTrip.dropoff.location.coordinates[0],
            activeTrip.dropoff.location.coordinates[1],
          ],
        },
  ...
  const updateLocationToBackend = useCallback(
    async (latitude, longitude) => {
      ...
      const timeSinceLastUpdate =
        now - (lastSentCoords.current?.timestamp || 0);
    ...
    try {
      ...
      const tripSummaryData = {
        ...
        startTime:
          ...
          new Date(Date.now() - 15 * 60000).toISOString(), // Default to 15 min ago if no start time
        distance:
          result.ride?.distance ||
          activeTrip.distance ||
          Math.random() * 10 + 2, // Use backend data if available
  ...
  const handlePickupDelivery = useCallback(async () => {
    ...
    try {
      ...
      if (
        activeTrip.allPickupDeliveries &&
        activeTrip.allPickupDeliveries.length > 1
      ) {
        console.log(
          "🚚 Confirming pickup for multiple deliveries:",
          activeTrip.allPickupDeliveries.length
        );

        // Use batch update for multiple deliveries
        const deliveryIds = activeTrip.allPickupDeliveries.map(
          (delivery) => delivery.deliveryId
        );
        console.log("🚚 Delivery IDs for batch update:", deliveryIds);

        const response = await updateMultipleDeliveryStatuses(
          deliveryIds,
          "started",
          userToken
        );
        ...
        console.log(
          `🚚 ${response.batchUpdate.totalUpdated} pickup deliveries confirmed successfully`
        );
      } else {
      ...
      setLocationError(
        "Failed to confirm pickup: " + (err.message || "Unknown error")
      );
    } finally {
      setIsUpdatingRideStatus(false);
    }
    ...
    try {
      ...
      if (onDeliveryConfirmation) {
        ...
        const response = await fetch(
          `${config.apiBaseUrl}/api/deliveries/confirm-delivery/${activeTrip._id}`,
          {
            method: "POST",
            headers: {
  ...
  const handleRatingSubmission = useCallback(
    async (ratingData) => {
      try {
  ...
  useEffect(() => {
    ...
    if (activeTrip && currentDestination) {
      ...
      if (
        !statusChanged &&
        now - lastBoundsFitTime.current < BOUNDS_FIT_THROTTLE
      ) {
        console.log(
          "Bounds fitting throttled - preventing mobile panning issue"
        );
        return;
      }
      ...
      if (isActiveTripWithFrequentUpdates && lastSentCoords.current) {
        ...
        if (distanceMoved < 50) {
          // Only update bounds if moved more than 50 meters
          console.log(
            "Skipping bounds fitting - user hasn't moved significantly"
          );
          return;
        }
      }
      ...
      console.log(
        "Fitting bounds for active trip - user location and destination:",
        currentTripStatus,
  ...
  useEffect(() => {
    ...
    if (isDeliveryUser && currentDelivery && currentDeliveryDestination) {
      ...
      console.log(
        "Fitting bounds for delivery trip - user location and destination:",
        deliveryStatus
      );
      ...
      lastBoundsFitTime.current = now;
      console.log(
        "Map bounds fitted to show user location and delivery destination"
      );
    }
  }, [
    map,
    ...
    const handleLocationSuccess = async (position) => {
      ...
      const shouldUpdateState =
        ...
        getDistanceMeters(
          ...
          latitude,
          longitude
        ) > (activeTrip ? 15 : 5); // Conservative threshold during trips
  ...
  useEffect(() => {
    ...
    if (!isCurrentTripDelivery) {
      ...
      if (
        ...
        console.log(
          "Showing start ride popup - distance to pickup:",
          distanceToPickup
        );
      ...
      if (
        ...
        console.log(
          "Showing complete ride popup - distance to dropoff:",
          distanceToDropoff
        );
    ...
    if (isCurrentTripDelivery) {
      ...
      if (
        ...
        console.log(
          "🚚 Showing pickup delivery popup - distance to pickup:",
          distanceToPickup
        );
        console.log(
          "🚚 Multiple deliveries from pickup:",
          activeTrip?.allPickupDeliveries?.length || 1
        );
        console.log("🚚 ActiveTrip data for pickup popup:", {
          activeTrip,
          allPickupDeliveries: activeTrip?.allPickupDeliveries,
          productDetailsStructure: activeTrip?.allPickupDeliveries?.map(
            (d) => ({
              orderId: d.orderId,
      ...
      if (
        ...
        console.log(
          "🚚 Driver arrived at delivery destination - notifying buyer"
        );
        ...
        if (isCurrentTripDelivery && onBuyerNotification && activeTrip?._id) {
          console.log(
            "🚀 Triggering dropoff_arrived notification for:",
            activeTrip._id
          );
          setTimeout(() => {
            onBuyerNotification(activeTrip._id, "dropoff_arrived")
              .then(() => {
                console.log(
                  "✅ Buyer notified of driver arrival with PIN information (non-intrusive)"
                );
              })
              .catch((err) => {
                console.error(
                  "❌ Failed to notify buyer of driver arrival:",
                  err
                );
              });
        ...
        console.log(
          "Showing complete delivery popup - distance to dropoff:",
          distanceToDropoff
        );
    ...
    if (
      ...
      updateFunction()
        .then(() => {
          ...
          console.log(
            `Auto-updated ${
              isCurrentTripDelivery ? "delivery" : "ride"
            } status to arrived - distance:`,
            distanceToPickup
          );
        })
        .catch((err) => {
  ...
  const [distanceToDeliveryPickup, setDistanceToDeliveryPickup] =
    useState(null);
  const [distanceToDeliveryDropoff, setDistanceToDeliveryDropoff] =
    useState(null);
  ...
  useEffect(() => {
    ...
    const distanceToPickup = getDistanceMeters(
      userLocation.lat,
      userLocation.lng,
      deliveryPickupCoords.lat,
      deliveryPickupCoords.lng
    );
    const distanceToDropoff = getDistanceMeters(
      userLocation.lat,
      userLocation.lng,
      deliveryDropoffCoords.lat,
      deliveryDropoffCoords.lng
    );
    ...
    if (
      ...
      distanceToPickup <= ARRIVAL_THRESHOLD &&
      !hasShownDeliveryPickupPopup.current
    ) {
      console.log(
        "Showing delivery start popup - distance to pickup:",
        distanceToPickup
      );
    ...
    if (
      ...
      console.log(
        "Showing pickup complete popup - distance to pickup:",
        distanceToPickup
      );
    ...
    if (
      deliveryStatus === "picked_up" &&
      distanceToDropoff <= ARRIVAL_THRESHOLD &&
      !hasShownDeliveryDropoffPopup.current
    ) {
      ...
      if (onBuyerNotification && currentDelivery?.deliveryId) {
        onBuyerNotification(currentDelivery.deliveryId, "dropoff_arrived")
          .then(() => {
            console.log(
              "✅ Buyer notified of driver arrival at dropoff location (non-intrusive)"
            );
          })
          .catch((err) => {
      ...
      console.log(
        "Showing delivery complete popup - distance to dropoff:",
        distanceToDropoff
      );
  ...
  useEffect(() => {
    ...
    const previousDeliveryId =
      previousActiveTrip.current?.deliveryId || previousActiveTrip.current?._id;

    if (
      currentDeliveryId &&
      previousDeliveryId &&
      currentDeliveryId !== previousDeliveryId
    ) {
      ...
      if (
        !isPopupInteractionActive &&
        (deliveryActionType !== "delivery" || !showDeliveryActionPopup)
      ) {
        setShowDeliveryActionPopup(false);
    ...
    directionsServiceRef.current.route(
      ...
      (result, status) => {
        if (status === window.google.maps.DirectionsStatus.OK) {
          ...
          console.log(
            "Navigation instructions extracted for destination:",
            currentDestination
          );
        } else {
    ...
    directionsServiceRef.current.route(
      ...
      (result, status) => {
        if (status === window.google.maps.DirectionsStatus.OK) {
          ...
          console.log(
            "Delivery directions calculated for destination:",
            currentDeliveryDestination
          );
        } else {
  ...
  const SlideToConfirm = memo(
    ({
      ...
      const progressPercentage =
        maxPosition > 0 ? (position / maxPosition) * 100 : 0;

      return (
        <div
          ref={containerRef}
          className={`slide-to-confirm ${variant} ${
            disabled ? "disabled" : ""
          }`}
        ...
        >
          ...
          <div
            ref={sliderRef}
            className={`slide-handle ${disabled ? "disabled" : ""} ${
              isDragging ? "dragging" : ""
            }`}
            style={{
              left: `${4 + position}px`,
  ...
  return (
    <div className="map-container">
            ...
            <div className="delivery-summary">
              ...
              <div className="orders-summary">
                ...
                <div className="orders-list">
                  {(activeTrip?.allPickupDeliveries?.length > 0
                    ? activeTrip.allPickupDeliveries
                    : [activeTrip]
                  ).map((delivery, index) => (
                    <div
                      key={delivery?.deliveryId || delivery?._id || index}
                      className="order-item"
                    >
                      <div className="order-badge">
                        <span className="order-number">
                          #{delivery?.orderId || `ORDER-${index + 1}`}
                        </span>
                        <span className="customer-name">
                          →{" "}
                          {delivery?.customer?.name ||
                            activeTrip?.customer?.name ||
                            activeTrip?.rider?.fullName ||
                            "Customer"}
                        </span>
                      </div>
                ...
                {activeTrip?.allPickupDeliveries?.length > 1 && (
                  <div className="total-orders">
                    <strong>
                      Total: {activeTrip.allPickupDeliveries.length} orders
                    </strong>
                  </div>
                )}
              </div>
      ...
      {showCompleteDeliveryPopup && isCurrentTripDelivery && (
        <div className="ride-action-popup" style={{ zIndex: 9999 }}>
          <div className="popup-content">
            ...
            <p>
              You've arrived at the delivery location. The customer will provide
              a PIN to confirm delivery.
            </p>
      ...
      {showDeliveryActionPopup && deliveryActionType && (
        <div
          className="ride-action-popup delivery-action-popup"
          style={{ zIndex: 9999 }}
        >
          <div className="popup-content">
            {deliveryActionType === "start" && (
              <>
                ...
                <p>
                  You've arrived at the pickup location. Ready to start the
                  delivery?
                </p>
                <SlideToConfirm
            ...
            {deliveryActionType === "delivery" && (
              <>
                <h3>🏠 Complete Delivery?</h3>
                <p>
                  You've arrived at the delivery location. Ready to complete the
                  delivery?
                </p>
                <SlideToConfirm
                  onConfirm={handleCompleteDelivery}
          ...
          <div className="navigation-instructions-compact">
            <div className="next-instruction-main">
              <div className="instruction-icon-large">
                {navigationInstructions[0]?.maneuver === "turn-left"
                  ? "⬅️"
                  : navigationInstructions[0]?.maneuver === "turn-right"
                  ? "➡️"
                  : navigationInstructions[0]?.maneuver === "straight"
                  ? "⬆️"
                  : navigationInstructions[0]?.maneuver === "merge"
                  ? "🔀"
                  : navigationInstructions[0]?.maneuver === "ramp"
                  ? "↗️"
                  : "⬆️"}
              </div>
              <div className="instruction-details">
                <div className="instruction-text-main">
                  {navigationInstructions[0]?.instruction ||
                    "Continue straight"}
                </div>
                <div className="instruction-distance-main">
                  {navigationInstructions[0]?.distance}
                </div>
              </div>
            </div>
...
const arePropsEqual = (prevProps, nextProps) => {
  ...
  if (
    prevTrip?._id !== nextTrip?._id ||
    prevTrip?.status !== nextTrip?.status
  ) {
    return false;
  }
  ...
  if (nextProps.activeTrip) {
    console.log(
      "Skipping marker comparison during active trip to prevent re-renders"
    );
    return true;
  }
  ...
  if (
    prevProps.markers?.[0]?.position?.lat !==
      nextProps.markers?.[0]?.position?.lat ||
    prevProps.markers?.[0]?.position?.lng !==
      nextProps.markers?.[0]?.position?.lng
  ) {
    return false;
  }
