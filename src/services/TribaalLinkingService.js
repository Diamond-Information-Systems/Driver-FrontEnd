...
class TribaalLinkingService {
  constructor() {
    ...
    this.endpoints = {
      linkAccount: "/api/account-linking/link-account",
      linkStatus: "/api/account-linking/link-status",
      walletBalance: "/api/wallet/balance-enquiry",
      processPayment: "/api/account-linking/wallet/payment",
      syncWallet: "/api/account-linking/wallet/sync",
      disconnect: "/api/account-linking/disconnect",
    };
  ...
  async linkTribaalAccount(email, password, token) {
    try {
      ...
      const response = await fetch(
        `${this.baseURL}${this.endpoints.linkAccount}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  async getLinkStatus(token) {
    try {
      const response = await fetch(
        `${this.baseURL}${this.endpoints.linkStatus}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  async disconnectAccount(token, reason = "User requested") {
    try {
      ...
      const response = await fetch(
        `${this.baseURL}${this.endpoints.disconnect}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  async syncWallet(token) {
    try {
      ...
      const response = await fetch(
        `${this.baseURL}${this.endpoints.syncWallet}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  async getWalletBalance(token, mobile) {
    const response = await fetch(`${this.baseURL}${this.endpoints.walletBalance}`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
  ...
  async processPayment(
    ...
    try {
      const response = await fetch(
        `${this.baseURL}${this.endpoints.processPayment}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
